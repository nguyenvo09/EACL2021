...
class HierachicalMultiHeadAttentionModel(BasicFCModel):
    ...
    def __init__(self, params):
        ...
        if self.use_claim_source: evd_input_size += self.claim_emb_size
        evd_input_size += dim * self.num_att_heads_for_words * self.num_att_heads_for_evds  # twice times for two times attention
        if self.use_article_source: evd_input_size += self.article_emb_size * self.num_att_heads_for_evds
    ...
    def forward(self, query: torch.Tensor, document: torch.Tensor, verbose = False, **kargs):
        ...
        assert n == 30
        query_repr = self._generate_query_repr(query, **kargs)  # output's shape is always (B1, D)
        # for documents
        ...
        bilstm_out = torch_utils.auto_rnn(self.bilstm, input_feats = embed_doc, lens = d_lens,
                                          new_indices = d_new_indices, restoring_indices = d_restoring_indices,
                                          max_len = self.fixed_length_right)  # (n1 + n2 + n3 + ... n_b, R, H)
        # Step 1: word-level attention
        avg, word_att_weights = self._word_level_attention(left_tsr=query_repr, right_tsr=bilstm_out,
        ...
        if kargs.get(KeyWordSettings.OutputRankingKey, False): return phi, (word_att_weights, evd_att_weight)
        return phi
    ...
    def _get_word_attention_func(self, dim: int):
        ...
        """
        input_dim = 2 * dim
        self.self_att_word = ConcatNotEqualSelfAtt(inp_dim=input_dim, out_dim=dim, num_heads=self.num_att_heads_for_words)
        # else:
